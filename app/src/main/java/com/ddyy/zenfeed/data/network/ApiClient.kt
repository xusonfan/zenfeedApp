...
object ApiClient {
     ...
     * @return ApiService实例
     */
    private suspend fun createApiService(baseUrl: String, context: Context): ApiService {
     ...
     * 创建HTTP客户端，包含代理配置
     */
    private suspend fun createHttpClient(context: Context): OkHttpClient {
        val settingsDataStore = SettingsDataStore(context)
     ...
     * @param context 上下文
     */
    suspend fun refreshApiService(context: Context) {
        synchronized(this) {
            _apiService = null
            _currentBaseUrl = ""
            _currentProxyConfig = ""
     ...
     */
    suspend fun getHttpClient(context: Context): OkHttpClient {
